...
fn main() {
    ...
    fn redraw_full(stdout: &mut io::Stdout, messages: &Vec<String>, scroll_offset: usize, status_msg: &str) -> (u16,u16) {
        ...
        if bar.len() > w as usize { bar.truncate(w as usize); }
        ...
        for (i, msg) in messages.iter().enumerate().skip(start) {
            ...
            let mut line = msg.replace('\n', "\\n"); if line.len() > w as usize { line.truncate(w as usize); }
    ...
    fn redraw_input(stdout: &mut io::Stdout, input: &str) {
        ...
        let mut prompt = format!("> {}", input); if prompt.len() > w as usize { prompt.truncate(w as usize); }
        let _ = write!(stdout, "{:<width$}", prompt, width = w as usize);
        queue!(stdout, cursor::MoveTo((2+input.chars().count()) as u16, y), cursor::Show).ok();
    }
    ...
    let mut status_msg = String::from("TUI started. Type /open <port> or /connect <addr:port>. /exit to quit.");
                            ...
                            KeyCode::Enter => {
                                ...
                                match parts.get(0).map(|s| s.as_str()) {
                                    ...
                                    Some("/connect") => {
                                        ...
                                        } else { status_msg = "Usage: /connect <addr:port>".into(); draw_state.force_full = true; }
                                    }
                                    Some("/exit") => {
...
fn run_network(tx_main: Sender<String>, rx_thread: Receiver<String>) {
    ...
    loop {
        ...
        while let Ok(cmd) = rx_thread.try_recv() {
            ...
            if let Some(rest) = cmd.strip_prefix("/open ") {
                ...
                match TcpStream::connect(rest) {
                    Ok(s) => { let s=s; s.set_nonblocking(true).ok(); clients.push(s); decoders.push(protocol::Decoder::new()); peer_meta.push(None); tx_main.send(format!("Connected to {} (id={})", rest, clients.len()-1)).ok(); }
                ...
                for (i, c) in clients.iter().enumerate() {
                    ...
                    lines.push(format!("id={} addr={} {}", i, addr, fp));
                }
        ...
        if let Some(l) = &listener {
            loop {
                match l.accept() {
                    Ok((s, peer)) => { let s=s; s.set_nonblocking(true).ok(); clients.push(s); decoders.push(protocol::Decoder::new()); peer_meta.push(None); tx_main.send(format!("Accepted {} (id={})", peer, clients.len()-1)).ok(); }
