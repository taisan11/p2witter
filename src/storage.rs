...
fn db_opt() -> Option<&'static Db> { DB.get() }

fn date_string(ts_millis: u64) -> String {
    use chrono::{TimeZone, Utc};
    // ts is unix millis UTC
    let secs = (ts_millis / 1000) as i64;
    let dt = Utc.timestamp_opt(secs, ((ts_millis % 1000) * 1_000_000) as u32).single().unwrap();
    format!("{:04}{:02}{:02}", dt.year(), dt.month(), dt.day())
}

fn encode_count(n: u64) -> [u8;8] { n.to_be_bytes() }
fn decode_count(b: &[u8]) -> u64 { if b.len()==8 { u64::from_be_bytes(b.try_into().unwrap()) } else { 0 } }
...
pub enum MsgKind { Chat, Dm, System }
...
pub fn append_message(ts_millis: u64, text: &str) {
    ...
    let current = db.get(&cnt_key).ok().flatten().map(|v| decode_count(&v)).unwrap_or(0);
    ...
    if current == 0 {
        ...
        let idx_key = b"index";
        let mut dates: Vec<String> = db.get(idx_key).ok().flatten().map(|v| {
...
pub fn store_structured(rec: &MessageRecord) -> Result<(), Box<dyn std::error::Error>> {
    let Some(db) = db_opt() else { return Ok(()); };
...
pub fn load_structured_day(date: &str) -> Vec<MessageRecord> {
    let Some(db) = db_opt() else { return Vec::new(); };
    let cnt_key = format!("cnt:{}", date);
    let total = db.get(&cnt_key).ok().flatten().map(|v| decode_count(&v)).unwrap_or(0);
    let mut out = Vec::new();
    for i in 0..total {
        let key = format!("{}{}", date, i);
        if let Ok(Some(val)) = db.get(key.as_bytes()) {
            if let Ok((rec, _len)) = bincode::serde::decode_from_slice::<MessageRecord, _>(&val, bincode::config::standard()) {
                out.push(rec);
            } else {
                ...
                if let Some(pos) = s.find('|') {
                    ...
                    out.push(MessageRecord { ts_millis: ts, recv_ts_millis: ts, kind: MsgKind::System, from_peer_id: None, to_peer_id: None, handle: None, text: txt, signed_ok: None });
                }
            }
        }
    }
...
pub fn list_dates() -> Vec<String> {
    let Some(db) = db_opt() else { return Vec::new(); };
    db.get(b"index").ok().flatten().map(|v| {
        String::from_utf8_lossy(&v).split('\n').filter(|s| !s.is_empty()).map(|s| s.to_string()).collect()
    }).unwrap_or_default()
}